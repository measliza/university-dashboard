...
const DepartmentSection = () => {
    ...
    const handleAddDepartment = () => {
        const newDepartment = {
            id: `${Date.now()}`,
            title: `Department ${department.length + 1}`,
            subtitle: "",
            linkTitle: "",
            urlLink: "",
            logo: "",
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
        <Droppable droppableId="droppable">
            {(provided) => (
                <div {...provided.droppableProps} ref={provided.innerRef} className='mx-4 mb-4'>
                    <ul class="h-auto  overflow-y-auto border rounded-t-lg mt-1">
                        {department.map((departments, index) => (
                            <Draggable
                                ...
                                {(provided) => (
                                    <li
                                        className={`below-border ${index === department.length - 1 ? 'border-none' : ''}`}
                                        ...
                                        <details className='group [&_summary::-webkit-details-marker]:hidden '>
                                            <summary className='flex justify-between rounded-lg px-2 py-2 pl-5 w-full '
                                                ...
                                                >
                                                <div className="flex ">
                                                    <svg class=" size-5 my-auto" viewBox="0 0 320 512" xmlns="http://www.w3.org/2000/svg">
                                                    ...
                                                    <span className="ml-2 text-lg">{departments.title}</span>
                                                </div>
                                                <span className='shrink-0 transition-transform duration-500 group-open:-rotate-0 flex gap-2'>
                                                    ...
                                                    <span
                                                        className={`shrink-0 transition-transform duration-300 ${
                                                                rotatedStates[departments.id] ? "rotate-180" : ""
                                                                }`}
